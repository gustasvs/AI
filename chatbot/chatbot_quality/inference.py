...
def do_start_inference(out_dir, hparams):
    ...
    hparams.infer_mode = "beam_search" # TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO
    ...
    with infer_model.graph.as_default():
        ...
        while True:
            try:
                ...
                # print(nmt_outputs, '- ORIGINAL NMT OUTPUTS')
                if hparams.infer_mode != "beam_search":
                    nmt_outputs = nmt.inference.nmt_model.np.expand_dims(nmt_outputs, 0)

                batch_size = nmt_outputs.shape[1] # Cik daudz jautājumi
                
                for sent_id in range(batch_size): # Iterate through jautajumiem
                    
                    translations = []
                    for beam_id in range(num_translations_per_input):
                        # print(f"translations per input - {num_translations_per_input}, hparams.infer_mode = {hparams.infer_mode}")
                        if hparams.eos:
                            tgt_eos = hparams.eos.encode("utf-8")

                        # Select a sentence
                        # print(nmt_outputs, '- NMT OUTPUTS')
                        # print(beam_id, sent_id)
...
if __name__ == "__main__":
    ...
    if sys.stdin.isatty() == False:
        ...
        # Process questions
        questions = sys.stdin.readlines()
        answers_list = process_questions(questions)

        # Print answers, but sexy style...
        def coolprinting(questions, answers_list):
            max_ans = 0; max_quest = 0;
            for index, answer in enumerate(answers_list):
                quest = str(questions[index])
                if len(quest) > max_quest: max_quest = len(quest)
                answ = str(answer['answers'][answer['best_index']])
                if len(answ) > max_ans: max_ans = len(answ)
            maxgar = max_ans + 9 + max_quest
            print('+', end=''); print((maxgar) * "-", end='+'); print(' ')
            for index, answer in enumerate(answers_list):
                quest = str(questions[index])[:-1]
                answ = str(answer['answers'][answer['best_index']])
                print(f"| {quest}", " " if (max_quest - len(quest)) % 2 == 1 else '', end=''); print("- " * ((max_quest - len(quest)) // 2), end='')
                print(f"- -> {answ}", ((maxgar - len(quest) - len(answ) - 9) - (max_quest - len(quest))) * " " ,"|")
            print('+', end=''); print((maxgar) * "-", end='+')

        coolprinting(questions, answers_list)

        # Print answers, but casual style... 
        # def casualprinting(answers_list):
            # for index, answer in enumerate(answers_list):
            #     print(answer['answers'][answer['best_index']])
        # casualprinting(answers_list)
        sys.exit()
    ...
    while True:
        ...
        # print(answers)
        if answers is None:
            print(colorama.Fore.RED + "No answers, daaawg..." + colorama.Fore.RESET)
        else:
            ...
            #     print(i, textstat.flesch_reading_ease(i))
os.chdir(original_cwd)
